...
#Usage: ma_intersect.rb <ma file 1> ... <ma file n> <use stdout>
...
class Array
    ...
    def tail(start=0)
        tail_len = self.length - start
        return self.slice(start,tail_len)
    end
end
...
use_stdout = nil
boolcount = ARGV.count("true") + ARGV.count("false")
if(boolcount > 1)
    puts "ERROR: Too many boolean arguments. (#{boolcount} for 1)"
    exit
end
if(ARGV.include?("true"))
    ARGV.delete("true")
    use_stdout = true
elsif(ARGV.include?("false"))
    ARGV.delete("false")
    use_stdout = false
end
ARGV.each_with_index { |ma_filename,i|
    ...
    while(ma_dataline = ma_filehandl.gets)
        if(!use_stdout && total_line_count % 1000 == 0)
            print "."
            $stdout.flush
        end
